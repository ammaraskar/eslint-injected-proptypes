...
const ast = require('eslint-plugin-react/lib/util/ast');
const variable = require('eslint-plugin-react/lib/util/variable');
const annotations = require('eslint-plugin-react/lib/util/annotations');
const versionUtil = require('eslint-plugin-react/lib/util/version');
const propsUtil = require('eslint-plugin-react/lib/util/props');
...
const BIND_ACTION_METHOD = "bindActionCreators";
  ...
  create: Components.detect((context, components, utils) => {
    ...
    */
    ...
    function getComponentName(node) {
      if (node && node.type === 'ClassDeclaration' && node.id) {
        return node.id.name;
      }
    ...
    function getReturnStatement(node) {
      if (ast.findReturnStatement(node)) {
        return ast.findReturnStatement(node).argument;
      } else if (node.type === 'FunctionDeclaration') {
        return ast.findReturnStatement(node.body).argument;
      } else if (node.type === 'ArrowFunctionExpression') {
        return node.body;
      }
    };

    function processReduxFunction(node, component) {
      if (node && node.init) {
        node = node.init;
      }
      if (!node) {
      ...
      if (
        ...
        returnStatement.callee.name === BIND_ACTION_METHOD
      ) {
        processReduxFunctionReturn(returnStatement.arguments[0], component);
        return;
      }
      processReduxFunctionReturn(returnStatement, component);
    }

    function processReduxFunctionReturn(node, component) {
      ...
      node.properties.forEach(prop => {
        if (prop.key && prop.key.type === 'Identifier') {
          ...
          declaredPropTypes[propName] = true;
        }
      });

      components.set(component.node, { declaredPropTypes });
    }

    function findVariableByName(context, name) {
      const vari = variable.getVariable(context.getScope().variables, name);
      if (!vari || !vari.defs[0] || !vari.defs[0].node) {
        return null;
      }
    
      if (vari.defs[0].node.type === 'TypeAlias') {
        return vari.defs[0].node.right;
      }
    
      return vari.defs[0].node;
    }

    // Given a node like:
    //   wrapper(wrapper2(wrapper3(Inside)))
    // resolve the `Inside` argument
    function digDownWrapperChain(node) {
    ...
    return {
      CallExpression: function(node) {
        if (node.callee.name != CONNECT_METHOD) {
          return;
        }
        const mapStateToPropsArg = node.arguments[0];
        const mapDispatchToPropsArg = node.arguments[1];

        let targetComponent = node.parent.arguments[0];
        if (!targetComponent) {
          return;
        }
        if (targetComponent.type === 'CallExpression') {
          targetComponent = digDownWrapperChain(targetComponent);
        }

        const component = Object.values(components.list()).find(
          c => getComponentName(c.node) == targetComponent.name
        );
        if (!component) {
          return;
        }

        // console.log("mapStateToProps", mapStateToPropsArg);
        if (mapStateToPropsArg && mapStateToPropsArg.type === 'Identifier' && mapStateToPropsArg.name) {
          const mapStateToProps = findVariableByName(context, mapStateToPropsArg.name);
          processReduxFunction(mapStateToProps, component);
        } else if (mapStateToPropsArg) {
          processReduxFunction(mapStateToPropsArg, component);
        }

        // console.log("mapDispatchToProps", mapDispatchToPropsArg);
        if (mapDispatchToPropsArg && mapDispatchToPropsArg.type === 'Identifier' && mapDispatchToPropsArg.name) {
          const mapDispatchToProps = findVariableByName(context, mapDispatchToPropsArg.name);
          processReduxFunction(mapDispatchToProps, component);
        } else if (mapDispatchToPropsArg && mapDispatchToPropsArg.type === 'ObjectExpression') {
          processReduxFunctionReturn(mapDispatchToPropsArg, component);
        } else {
          processReduxFunction(mapDispatchToPropsArg, component);
        }
      },
